...
def create_query_routes(rag, api_key: Optional[str] = None, top_k: int = 60):
    ...
    async def call_agent_generate_survey(request: QueryRequest):
        """
        Streams CoT and final result from the agent running at localhost:8001/generate_survey.
        Emits reasoning steps as "[Chain of Thought]: ..." and the final result as-is.
        """
        async with httpx.AsyncClient(timeout=120.0) as client:
            async with client.stream(
                "POST",
                "http://localhost:8001/generate_survey",
                json=request.dict()
            ) as resp:
                ...
                async for line in resp.aiter_lines():
                    ...
                    try:
                        ...
                        if "step" in data:
                            yield f"[Chain of Thought]: {data['step']}"
                        elif "response" in data:
                            yield data["response"]
                        else:
                            yield f"[Agent] {line.strip()}"
                    except json.JSONDecodeError:
                        yield f"[Invalid JSON] {line.strip()}"

    from fastapi.responses import StreamingResponse
    @router.post("/query/stream", dependencies=[Depends(combined_auth)])
    async def query_text_stream(request: QueryRequest):
        """
        Streams reasoning and final survey from the local LLM agent.
        Emits all steps under "response" key to align with frontend chat display.
        """
        try:
            # Don't await — it's an async generator
            response = call_agent_generate_survey(request)
